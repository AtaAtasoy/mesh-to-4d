...
class SaverMixin:
    ...
    def save_guidance_eval_images(self, guidance_eval_out: Dict[str, torch.Tensor], filename: str, step: int) -> None:
        imgs_noisy = guidance_eval_out["imgs_noisy"]
        imgs_1step = guidance_eval_out["imgs_1step"]
        imgs_1orig = guidance_eval_out["imgs_1orig"]
        imgs_final = guidance_eval_out["imgs_final"]
        
        save_image(tensor=imgs_noisy, fp=self.get_save_path(f"{filename}_noisy_{step}.png"))
        save_image(tensor=imgs_1step, fp=self.get_save_path(f"{filename}_1step_{step}.png"))
        save_image(tensor=imgs_1orig, fp=self.get_save_path(f"{filename}_1orig_{step}.png"))
        save_image(tensor=imgs_final, fp=self.get_save_path(f"{filename}_final_{step}.png"))
